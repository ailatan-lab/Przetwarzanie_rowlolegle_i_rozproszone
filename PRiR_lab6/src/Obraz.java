...
class Obraz {
    ...
    public void calculate_histogram_w1(int nr_w){

        for(int i=0;i<size_n;i++) {
            for(int j=0;j<size_m;j++) {
                if(tab[i][j] == tab_symb[nr_w])
                    hist_parallel[nr_w]++;
            }
        }
    ...
    public void calculate_histogram_w2(int start, int stop, int krok){

        for(int i=0;i<size_n;i++) {
            for(int j=0;j<size_m;j++) {
                for(int k=start;k<stop;k+=krok) {
...
//
    ...
    public void print_histogram_w1(int id_w){
        System.out.print("Wątek " + id_w + ": " + (char)(id_w+33) + " ");
        for(int i=0;i<hist_parallel[id_w];i++) {
            System.out.print("=");
        }
        System.out.println();
    ...
    public synchronized void print_histogram_w2(int id_w, int znak){
        System.out.print("Wątek " + id_w + ": " + (char)(znak+33) + " ");
        for(int i=0;i<hist_parallel[znak];i++) {
            System.out.print("=");
        }
        System.out.println();
    }

    public void check_histogram(){
        for(int i=0;i<94;i++) {
            if(histogram[i] != hist_parallel[i]) {
                System.out.println("Histogramy różne");
                return;
            }
        }
        System.out.println("Histogramy identyczne");
    }
